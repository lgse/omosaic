...
test("renderer preserves stock IPC and exposes per-screen methods", () => {
  ...
  assert.match(qml, /hyprpicker --autocopy --format=hex --no-fancy --quiet/)
  assert.match(qml, /wl-paste --no-newline/)
  assert.match(qml, /pickColorForKey/)
  assert.match(qml, /output\.match\(\/#\[0-9a-fA-F\]\{6\}/)
...
test("bar widget exposes per-display wallpaper controls", () => {
  ...
  assert.match(qml, /pickColorForKey/)
  assert.match(qml, /id: colorPickerDelay/)
  assert.match(qml, /interval: 250/)
  assert.match(qml, /label: "󰃉"/)
  ...
  assert.match(qml, /Presets: 0° → right · 90° → down/)
  assert.match(qml, /chooseFileForKey/)
